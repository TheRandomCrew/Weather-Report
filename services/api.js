...
export const getWeatherByCityId = async (cityId) => {
  return await axios.get(`${OPENWEATHERMAP_BASE_URL}?id=${cityId}&appid=${OPENWEATHERMAP_API_KEY}`)
}
...
export const getWeatherByCoords = async (lat, lon) => {
  return await axios.get(`${OPENWEATHERMAP_BASE_URL}?lat=${lat}&lon=${lon}&appid=${OPENWEATHERMAP_API_KEY}`)
}
